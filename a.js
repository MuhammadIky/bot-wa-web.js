...
// client.on("ready", () => {
//	     console.log(`[ ${moment().format("HH:mm:ss")} ] BOT UPDATE READY TO SEND !`
//		 );
//    fs.readFile("./CoronaService/userupdate.json", "utf-8", function(err, data) {
//    if (err) throw err;
//    const userData = JSON.parse(data);
//    for (var i = 0; i < userData.length; i++) {
//      let number = userData[i].user;
// client.sendMessage(number, `Hi *${number}* 😁.
//`);
// }
// });
//	});
...
client.on('group_join', async (notification) => {
   ...
   // var i;
    //for (let participant of chat.participants) {
       ...
       // if (participant.isAdmin) {
           ...
           // const media = MessageMedia.fromFilePath('./tts/member-baru.mp3');
 ...
 //   console.log('Admins: ', admins);
    //notification.reply('User leave'); // sends message to self
		});
...
client.on('group_leave', async (notification) => {
    ...
    const botno = notification.chatId.split('@')[0];
    let number = await notification.id.remote;
    client.sendMessage(number, `Hmm... Selamat tinggal kawan`);
   ...
   // var i;
    //for (let participant of chat.participants) {
       ...
       // if (participant.isAdmin) {
            ...
            //admins[contact.pushname] = participant.id.user;
           // client.sendMessage(participant.id._serialized, 'Hai admin, ada member yang keluar di group mu');
           // const media = MessageMedia.fromFilePath('./tts/member-keluar.mp3');
           // client.sendMessage(participant.id._serialized, media);
     //   }
  //  }
  //  console.log('Group Details');
...
Alasan: "${teks}"`);
			...
			const { hasMedia, hasQuotedMsg, isForwarded, body, type, from, author, mentionedIds, location } = msg
			if (chat.isGroup) {
				chat.removeParticipants(msg.from)
...
fetch("https://trace.moe/api/search", {
  ...
  .then(res => res.json())
  .then(result =>  {
var teks = `Hmm.. Ditemukan :)
What Anime Is That ?

Judul : *${result.docs[0].title}, ${result.docs[0].title_romaji}*
...
Yang ingin jasa pembuatan bot *GRATIS* bisa chat ke https://s.id/jasabot`, { mentions: [contact]});
	}
   else if (msg.body == ">1"){
