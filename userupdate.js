        ...
        fs.readFile('./CoronaService/userupdate.json', 'utf-8', function (err, data) {
            ...
            checkUser(user).then(result => {
                if (result) {
                    ...
                    fs.writeFile('./CoronaService/userupdate.json', JSON.stringify(parsed), (err) => {
                        if (err) reject(err)
                        console.log(`[ ${moment().format('HH:mm:ss')} ] Add User UPDATE ${user}`)
                        resolve(true)
                    })
                }
            })
    ...
    return new Promise((resolve, reject) => {
        fs.readFile('./CoronaService/userupdate.json', 'utf-8', function (err, data) {
            ...
            if (parsed.findIndex(x => x.user === nomor) !== undefined && parsed.findIndex(x => x.user === nomor) !== -1) {
                ...
                fs.writeFile('./CoronaService/userupdate.json', JSON.stringify(parsed), (err) => {
                    if (err) reject(err)
                    console.log(`[ ${moment().format('HH:mm:ss')} ] Delete User UPDATE ${nomor}`)
                    resolve(true)
                })
            }
